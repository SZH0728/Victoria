...
class EffectModifyRandomize(EffectModifyBase):
    ...
    def modify(self) -> Any:
        ...
        enable_random_technology = self.middle.get('enable_random_technology', False)

        random_technology_range: tuple[int, int] = self.middle.get('random_technology_range', (1, 6))
        if random_technology_range[0] > random_technology_range[1]:
            random_technology_range = (random_technology_range[1], random_technology_range[0])

        enable_random_law = self.middle.get('enable_random_law', False)

        technology: list[str] = [f'effect_starting_technology_tier_{i}_tech' for i in range(random_technology_range[0], random_technology_range[1] + 1)]
        technology_weight: list[int] = list(range(random_technology_range[0], random_technology_range[1] + 1))
        ...
        if enable_random_technology:
            effect_list.append(choices(technology, weights=technology_weight, k=1)[0])

        if enable_random_law:
